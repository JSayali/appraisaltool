                ...
                <c:if test="${method.charPracticeImplementation}">
                    <div>
                        ...
                        <c:if test="${empty practiceRule}">
                            ...
                            <form action="addpracticerule.do" method="get">
                        </c:if>
                        <c:if test="${!(empty practiceRule)}">
                                    ...
                                    <c:forEach items="${method.practiceImplementation}" var="scale">
                                        <td>
                                            <select name="${scale.id}-source">
                                                <option value="NO" <c:if test="${!(empty practiceRule) && strmik:isRuleCompletion(scale,practiceRule.rules,'NO',true)}">selected="selected"</c:if>> <f:message key="rule-NO" /> </option>
                                                <option value="YES" <c:if test="${!(empty practiceRule) && strmik:isRuleCompletion(scale,practiceRule.rules,'YES',true)}">selected="selected"</c:if>> <f:message key="rule-YES" /> </option>
                                                <option value="YES_NO" <c:if test="${!(empty practiceRule) && strmik:isRuleCompletion(scale,practiceRule.rules,'YES_NO',true)}">selected="selected"</c:if>> <f:message key="rule-YES_NO" /> </option>
                                            </select>
                                        </td>
                                    </c:forEach>
                                        <td>-&gt;</td>
                                    <c:forEach items="${method.practiceImplementation}" var="scale">
                                        <td>
                                            <select name="${scale.id}-target">
                                                <option value="NO" <c:if test="${!(empty practiceRule) && strmik:isRuleCompletion(scale,practiceRule.rules,'NO',false)}">selected="selected"</c:if>> <f:message key="rule-NO" /> </option>
                                                <option value="YES" <c:if test="${!(empty practiceRule) && strmik:isRuleCompletion(scale,practiceRule.rules,'YES',false)}">selected="selected"</c:if>> <f:message key="rule-YES" /> </option>
                                            </select>
                                        </td>
                                    </c:forEach>
                                    </tr>
                            ...
                            <tr>
                                <td><c:out value="${rule.ruleNo}" /></td>
                                    <c:forEach items="${method.practiceImplementation}" var="scale">
                                        ...
                                        <c:forEach items="${rule.rules}" var="aRule">
                                            <c:if test="${aRule.scale eq scale}">
                                                <f:message key="rule-${aRule.source}" />
                                    ...
                                    <c:forEach items="${method.practiceImplementation}" var="scale">
                                        ...
                                        <c:forEach items="${rule.rules}" var="aRule">
                                            <c:if test="${aRule.scale eq scale}">
                                                <f:message key="rule-${aRule.target}" />
                                                <c:set var="found" value="${true}" />
                                            </c:if>
                                        </c:forEach>
                                        <c:if test="${!found}">
                                ...
                                <td>
                                    ...
                                    <c:otherwise>
                                        ...
                                        | <a class="confirmable" href="<c:out value="deletepracticerule-${rule.id}.do" />"><f:message key="delete" /></a>
                                    </c:otherwise>
                                    </c:choose>
                                </td>
                            </tr>
