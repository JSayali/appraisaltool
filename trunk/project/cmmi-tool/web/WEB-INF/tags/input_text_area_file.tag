...
<%@attribute name="cols" required="false" description="default 35" %>
<%@attribute name="rows" required="false" description="default 5"%>

<c:if test="${empty cols}">
    <c:set var="cols" value="35" />
</c:if>
...
<div class="ctrlHolder <spring:bind path="${object}.${property}"><c:if test="${status.error}">error</c:if></spring:bind>">
    ...
    <c:if test="${!password}">
        <form:textarea path="${property}" id="${property}" cssClass="textInput" cols="${cols}" rows="${rows}" disabled="${disabled}" />
    </c:if>
</div>